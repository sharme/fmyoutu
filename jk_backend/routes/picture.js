...
function resize(filename, folder, customFolderPath, originalFolderPath, bigFolderPath, res, widthPixel, heightPixel) {
    ...
    easyimg.resize({
        ...
        //             }).then(
        //                 function (image) {
        //                     console.log('resize custom image: ' + image.width + " , customFolderPath: " + customFolderPath + "/" + filename);
        //                    
        //                     if(waterMark != ''){
        //                         gm(customFolderPath + "/" + filename)
        //                             .stroke("#F08080")
        ...
        //             }).then(
        //                 function (image) {
        //                     console.log('resize custom image: ' + image.width);
        //
        //                     if(waterMark != ''){
        //                         gm(customFolderPath + "/" + filename)
        //                             .stroke("#F08080")
    ...
    function uploadFile(folder, uptoken, targetCloudFolder, localFile) {
        ...
        qiniu.io.putFile(uptoken, targetCloudFolder, localFile, extra, function(err, ret) {
            if(!err) {
                ...
                // } else {
                    updateBigFileQiniu(folder, originalCloudFolder, originalPath, res, "http://o99spo2ev.bkt.clouddn.com/" + ret.key);
                // }
    ...
    function uploadFile(folder, uptoken, targetCloudFolder, localFile) {
        ...
        qiniu.io.putFile(uptoken, targetCloudFolder, localFile, extra, function(err, ret) {
            if(!err) {
                ...
                var data = {
                    "customImg": "http://o99spo2ev.bkt.clouddn.com/" + ret.key,
